...
class Student:
    ...
    def getCurrentSemester(self):
        admisionY = self._admissionYear
        admisionA = self._admissionSemester
        currentYear = 2019
        currentSemester = 2
        amountSemesters = 0
        
        #assumption that 2019 is the current year and semester 2
        amountSemesters = (currentYear - admisionY) * currentSemester

        if admisionA == 1:
            amountSemesters += 2

        return amountSemesters
    ...
    def printEnrolmentCertificate(self):        
        ...
        print(f'This is to certify that {title} {self._studentProfile.firstName + " " + self._studentProfile.lastName} with student id {self._studentProfile.studentId} is a student at semester {self.getCurrentSemester()} at CICCC.')
...
class PortalManager:
    ...
    def registerNewStudent(self, firstName, lastName, gender, country, address, admisionYear, age, username, password):
        # create a student
        sampleStudentProfile = StudentProfile(firstName.strip().capitalize() , lastName.strip().capitalize() , gender, country, age, random.randint(1000000, 99999999), address)
        ...
        if username.upper() == "REGISTER":            
            ...
            while exists == False:
                for student in students:
                    ...
                    if username.upper() == account.getUsername().upper() and password == account.getPassword():
                        print()
                        print('************************************************************')
